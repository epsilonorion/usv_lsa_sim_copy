...
namespace gazebo
{
	...
	void
	FreeFloatingControlPlugin::ReadVector3 (const std::string &_string, ignition::math::Vector3<double> &_vector)
	{
		...
		if (control_joints_ && model_->GetJointCount () != 0)
		{
			...
			for (i = 0; i < model_->GetJointCount (); ++i)
			{
				...
				if (control_node.hasParam (name))
				{
					...
					joint_min.push_back (joint->LowerLimit (0));
					joint_max.push_back (joint->UpperLimit (0));
	...
	FreeFloatingControlPlugin::Update ()
	{
		...
		if (controller_is_running_)
		{
			...
			{
				//std::cerr<<"\n cog: "<<body_->WorldCoGPose().Pos().Z()<<" water: "<<z_surface_;
				...
				if (thruster_fixed_idx_.size ())
				{
					...
					body_->AddForceAtWorldPosition (
					        body_->WorldPose ().Rot().RotateVector (ignition::math::Vector3<double> (fixed (0), fixed (1), fixed (2))),
					        body_->WorldCoGPose ().Pos());
					body_->AddRelativeTorque (ignition::math::Vector3<double> (fixed (3), fixed (4), fixed (5)));
				}
					...
					for (unsigned int i = 0; i < thruster_steer_idx_.size (); ++i)
						if (thruster_links_[i]->WorldCoGPose ().Pos().Z() < link_water_surface[i]->waterSurface.Z())
						{
							...
							if (thruster_command_ (thruster_steer_idx_[i]) < -thruster_max_command_[i])
							{
								...
								thruster_links_[i]->AddRelativeForce (ignition::math::Vector3<double> (0, 0, thruster_max_command_[i]));
							}
							...
							else
							{
								...
								thruster_links_[i]->AddRelativeForce (
								        ignition::math::Vector3<double> (0, 0, -thruster_command_ (thruster_steer_idx_[i])));
							}
						...
						else
						{
							...
							//std::cerr<<"\n THRUSTER: "<<thruster_links_[i]->GetName()<<" POS: "<<thruster_links_[i]->WorldPose();
							//std::cerr<<"\n ["<<i<<"]"<<thruster_links_[i]->GetName()<<"  :"<<thruster_links_[i]->WorldCoGPose ().Pos().Z()<<" < "<<link_water_surface[i]->waterSurface.Z();
							if (thruster_command_ (thruster_steer_idx_[i]) < -thruster_overWater_max_command_[i])
							{
								thruster_links_[i]->AddRelativeForce (
								        ignition::math::Vector3<double> (0, 0, thruster_overWater_max_command_[i]));
							}
