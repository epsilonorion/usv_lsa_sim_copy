...
Foil_Dynamics_Plugin::Foil_Dynamics_Plugin () : rho (1000.1)
{
	...
	this->cp = ignition::math::Vector3<double> (0, 0, 0);
	this->forward = ignition::math::Vector3<double> (1, 0, 0);
	this->upward = ignition::math::Vector3<double> (0, 0, 1);
	this->area = 1.0;
	this->mult_lift = 1.5;
	this->mult_drag = 1.0;
...
Foil_Dynamics_Plugin::Load (physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
	...
	if (_sdf->HasElement ("cp"))
		this->cp = _sdf->Get < ignition::math::Vector3<double> > ("cp");

	// blade forward (-drag) direction in link frame
	if (_sdf->HasElement ("forward"))
		this->forward = _sdf->Get < ignition::math::Vector3<double> > ("forward");

	// blade upward (+lift) direction in link frame
	if (_sdf->HasElement ("upward"))
		this->upward = _sdf->Get < ignition::math::Vector3<double> > ("upward");
	...
	waterCurrent = ignition::math::Vector3<double> (0, 0, 0);
	float wind_value_x;
	float wind_value_y;
	float wind_value_z;
	running = false;
	if (fluidVelocity.compare ("global") == 0)
	{
		...
		{
			this->wind = ignition::math::Vector3<double> (wind_value_x, wind_value_y, 0);
...
Foil_Dynamics_Plugin::OnUpdateRudder ()
{
	...
	ignition::math::Vector3<double> vel = this->link->WorldLinearVel (this->cp) - waterCurrent;
	...
	if (vel.Length () > 5)
	{
		...
		//std::cerr<<"\n cp "<<this->cp<<" pose: "<<this->link->WorldPose ();
		return;
	}
	// pose of body
	...
	ignition::math::Vector3<double> forwardI = pose.Rot().RotateVector (this->forward);
	ignition::math::Vector3<double> upwardI = pose.Rot().RotateVector (this->upward);
	//std::cerr<<"pose: "<<pose<<" forwardI: "<<forwardI<<" upwardI: "<<upwardI<<"\n";

	// ldNormal vector to lift-drag-plane described in inertial frame
	ignition::math::Vector3<double> ldNormal = forwardI.Cross (upwardI).Normalize ();


	// angle of attack
	ignition::math::Vector3<double> velInLDPlane = ldNormal.Cross (vel.Cross (ldNormal));

	// get direction of drag
	ignition::math::Vector3<double> dragDirection = -velInLDPlane;
	dragDirection.Normalize ();

	// get direction of lift
	ignition::math::Vector3<double> liftDirection = ldNormal.Cross (velInLDPlane);
	liftDirection.Normalize ();
	//std::cerr<<"vel: "<<vel<<"("<<vel.Length()<<") liftDirection: "<<liftDirection<<" dragDirection: "<<dragDirection<<"\n";

	// get direction of moment
	ignition::math::Vector3<double> momentDirection = ldNormal;
	...
	ignition::math::Vector3<double> lift = cl * q * this->area * liftDirection;
	...
	// drag at cp
	ignition::math::Vector3<double> drag = cd * q * this->area * dragDirection;

	// force and torque about cg in inertial frame
	ignition::math::Vector3<double> force = lift + drag;
...
//	std::cerr<<" totalForce: "<<(lift+drag)<<"\n";
	...
	ignition::math::Vector3<double> torque = (lift + drag)*(this->cp - this->link->GetInertial ()->CoG ());

	// apply forces at cg (with torques for position shift)
	//std::cerr<<"\n forceRudder["<<force.Length()<<"]: "<<force;
...
Foil_Dynamics_Plugin::OnUpdateKeel ()
{
	...
	ignition::math::Vector3<double> moment = cm * q * this->area * momentDirection;

	// moment arm from cg to cp in inertial plane
	ignition::math::Vector3<double> momentArm = pose.Rot().RotateVector (this->cp - this->link->GetInertial ()->CoG ());

	// force and torque about cg in inertial frame
	ignition::math::Vector3<double> force = lift + drag;
	// + moment.Cross(momentArm);

	ignition::math::Vector3<double> torque = moment;

	//std::cerr<<"\n forceKeel["<<force.Length()<<"]: "<<force;
	// apply forces at cg (with torques for position shift)
	this->link->AddForceAtRelativePosition (force, this->cp);
...
Foil_Dynamics_Plugin::OnUpdateSail ()
{
	...
	ignition::math::Vector3<double> aw = this->wind - this->link->WorldLinearVel (this->cp);
	if (aw.Length () <= 0.01)
		return;
	...
	// rotate forward and upward vectors into inertial frame
	ignition::math::Vector3<double> forwardI = pose.Rot().RotateVector (this->forward); //xb
	ignition::math::Vector3<double> upwardI = pose.Rot().RotateVector (this->upward);   //yb

	// ldNormal vector to lift-drag-plane described in inertial frame
	ignition::math::Vector3<double> ldNormal = forwardI.Cross (upwardI).Normalize ();
	// TODOS ESSES PRODUTOS VETORIAIS SÃO PRA PEGAR OS VETORES PERPENDICULARES

	//ignition::math::Vector3<double> velInLDPlane = ldNormal.Cross(aw.Cross(ldNormal)); // isso é igual ao vel, só que escalado????
	ignition::math::Vector3<double> velInLDPlane = aw;
	// get direction of drag
	ignition::math::Vector3<double> dragDirection = velInLDPlane;
	dragDirection.Normalize ();
	...
	this->link->AddForceAtRelativePosition (force, this->cp);
//std::cerr<<"\n force: "<<force;
//std::cerr<<"\n aw:"<<aw.Length ()<<", position: "<<this->cp;
//std::cerr<<"\n aw:"<<aw.Length ()<<" force: "<<force.Length();
//std::cerr<<"\n aw:"<<aw.Length ()<<" lift: "<<lift<<" drag: "<<drag;
//std::cerr<<"\n aw:"<<aw.Length ()<<" lift: "<<lift.Length ()<<" drag: "<<drag.Length ()<<" cl: "<<cl<<" area: "<<this->area<<" q: "<<q<<" rho: "<<this->rho<<" speedInLDPlane: "<<speedInLDPlane;
//std::cerr<<"\n aw: "<<aw;
}
...
Foil_Dynamics_Plugin::WaterThreadLoop ()
{
	...
	while (running)
	{
		ignition::math::Pose3<double> pose = this->link->WorldCoGPose ();
